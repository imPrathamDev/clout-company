# Taste
- Prefers rem-based units (16px = 1rem) over hardcoded px for layout values — widths, heights, padding, margin, gap, font-size, border-radius, positioning — so the whole site can be scaled globally via `html { font-size: X% }`. Confidence: 0.9
